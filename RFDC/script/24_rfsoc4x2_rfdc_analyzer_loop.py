...
def plot_spectrum(fpgas, cx=False):
  ...
  fig, ax = plt.subplots(1, 1, sharey = 'row', sharex='col')
  lines = []
  ...
  if cx: # combine real/imag from the two interfaces
    ...
    # remove extra 0's for LF 
    #y = y.reshape((4,-1,4))
    #y = y[:,0::2,:]
    #y = y.reshape((4,-1, Nfft))
    #Nsamps /= 2 
    # END remove extra 0's
  else:
    y = np.reshape(X, (Nant, Nsamps//Nfft, Nfft))

  w = signal.blackmanharris(Nfft)
  Y = fft.fft(y*w[None, None, :], Nfft, axis=2)
  ...
  if True:
      ...
      #line, = cur_ax.plot(y.real(), 'b-')
      #line, = cur_ax.plot(y.imag(), 'r-')
      lines.append(line)
      cur_ax.grid();
      cur_ax.set_ylim((40,150))
      cur_ax.set_title('port {:d}'.format(idx))

      #if i==(PLT_DEPTH-1):
      cur_ax.set_xlabel('freq (MHZ)')
      #if j==0:
...
if __name__=="__main__":
  ...
  fpga = casperfpga.CasperFpga('10.1.1.10')
  time.sleep(0.2)
